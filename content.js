const API_BASE = 'https://qa-grader-api.onrender.com';
...
function renderList(){
  ...
  list.innerHTML = pageData.items.map(t => {
    ...
    const pc = t.priority === 'high' ? 'tph' : t.priority === 'mid' ? 'tpm' : 'tpl';
    ...
    return `<div class="ti ${sel?.id === t.id ? 'on' : ''}" data-id="${t.id}">
      ...
      <div class="tmeta"><span class="tagt">${t.agent || '—'}</span><span class="tpb ${pc}">${t.priority}</span></div>
    </div>`;
  }).join('');
...
function pickTicket(id){
  ...
  renderList();
  if(sel && sel.frontUrl && !(sel.conv && sel.conv.length)){
...
function renderDetail(){
  ...
  document.getElementById('panel').innerHTML = `<div class="detail">
    ...
    }).join('') : `<div class="msg"><div>${t.frontUrl ? 'Loading conversation…' : 'No conversation imported'}</div></div>`}</div></div>
  ...
  return `
    ...
    <div class="tdm-sec">
      ...
      <div class="tdm-conv">${(t.conv && t.conv.length) ? t.conv.map(m => {
        ...
        const imgHtml = (m.imgs && m.imgs.length) ? m.imgs.map(a => `<img class="conv-img" data-att="${a.url}" alt="${a.name}" src="">`).join('') : '';
        return `<div class="msg${isNote ? ' msg-note' : ''}"><div class="mh"><span class="ms ${cls}">${label}</span><span class="mt">${m.t}</span></div><div class="msg-copy">${renderMessageBody(m.b)}</div>${imgHtml}</div>`;
      }).join('') : `<div class="msg"><div>${t.frontUrl ? 'Loading conversation…' : 'No conversation imported'}</div></div>`}</div>
    </div>
    <div class="tdm-sec">
...
(async function checkSession() {
  showBootLoading('Loading tickets and grades…');
  try {
